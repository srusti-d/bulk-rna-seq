...
design <- design[!apply(design, 1, function(row) any(row == "Missing")),]
...
# Interaction Model (pairwise)

# Initialize model with the first main effect
model <- paste("~", colnames(design)[1])

# Add each subsequent variable as main effects
...
for (i in 1:(length(colnames(design)) - 1)) {
    for (j in (i + 1):length(colnames(design))) {
        model <- paste(model, "+", colnames(design)[i], ":", colnames(design)[j])
    }
}
